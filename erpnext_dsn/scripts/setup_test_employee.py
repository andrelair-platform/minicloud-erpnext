...
for component in [
    ...
    {
        "salary_component": "Cotisation retraite",
        "salary_component_abbr": "RETR",
        "type": "Deduction",
        "is_tax_applicable": 0,
    },
    {
        "salary_component": "CSG/CRDS non déductible",
        "salary_component_abbr": "CSG",
        "type": "Deduction",
        "is_tax_applicable": 0,
    },
]:
...
if not frappe.db.exists("Holiday List Assignment", {"assigned_to": EMP_ID, "docstatus": 1}):
    hla = frappe.get_doc(
        {
...
if not SLIP_EXISTS:
    ...
    existing = frappe.db.get_value(
        ...
        ["name", "gross_pay", "net_pay"],
        as_dict=True,
    )
    print(
        f"Salary slip {existing.name} already exists — "
...
print("  POST /api/method/erpnext_dsn.api.submit_monthly_dsn?year=2026&month=1")
